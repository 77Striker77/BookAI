---
titel: "Die bösen Jungs"
titel_en: "The Bad Guys"
werk_typ: "reihe"
universum: "Vuldranni (iNcarn8)"
verwandte_reihen: ["[[Die guten Jungs]]"]
autor: "Eric Ugland"
jahr_start: 2019
erscheinungsdatum_bd1: "2019"        # EN „Scamps & Scoundrels"; dt. Hörbuch 2023
letztes_datum: "2024"                # Bd. 11 „2 Lies, 2 Thrones"
baende_gesamt: 11
baende_geplant: 12          # ⚠️ KORREKTUR 2026-08-11: Bd. 12 für 2026 angekündigt
abgeschlossen: false       # ⚠️ KORREKTUR: Reihe läuft weiter (Bd. 12)
angekuendigt: "Bd. 12 für 2026 angekündigt (Risingshadow) — Reihe NICHT abgeschlossen. Universum zusätzlich: The Grim Guys (EN)"
naechster_band_de: "Buch: alle 11 erschienen · HÖRBUCH: ⚠️ Produktion läuft (belegt bis Bd. 8), nächster Termin unbelegt"
naechster_band_en: "Bd. 12 — 2026 angekündigt, Titel/Termin offen"
sprachen_verfuegbar: [de, en]
sprache_erlebt: "de"
verlag_de: "LMBPN International"
verlag_hoerbuch_de: "Audible Studios"
uebersetzer: "Simone Förster (Bd. 1 zusätzlich Moritz Hirsch)"
rating_schnitt: "⚠️ Serienwert nicht belegt"
rating_stimmen_gesamt: "⚠️ nicht belegt"
bekanntheit_stufe: "Etabliert"
hype: "Parallelreihe der etablierten Good-Guys-Reihe"
kontext: "LitRPG"
verdict: "offen"                     # AUSGESETZT 2026-08-11 — vorher fälschlich "liked" (vom System gesetzt, nicht vom Nutzer)
verdict_status: "AUSGESETZT — Nutzer 2026-08-11: „kann ich erst nach Ende von Band 1 was zu sagen""
wiedervorlage: "Volles Interview fahren, sobald Bd. 1 durch ist (spoiler_erlebt_bis: 1). Vorher nicht nach einer Meinung fragen."
format_erlebt: "hoerbuch"
sprecher: "Thomas Nicolai"
performance: "solo"
genres: [LitRPG, GameLit, Action-Komödie]
themen: []
ton: [Absurder Humor, Tonwechsel Humor-Ernst]
erzaehlstil: []
tempo: "⚠️ offen"
komplexitaet: ""
figuren: [Pragmatischer Held mit klarem Kompass, Moralisch ambivalenter Antiheld]
setting: [Stimmige Weltatmosphäre]
handwerk: [Über-crunchiges Zahlensystem]
bewertung:
  - { aspekt: "Absurder Humor", wert: 2 }
  - { aspekt: "Tonwechsel Humor-Ernst", wert: 2 }
  - { aspekt: "Pragmatischer Held mit klarem Kompass", wert: 2 }
  - { aspekt: "Stimmige Weltatmosphäre", wert: 2 }
  - { aspekt: "Moralisch ambivalenter Antiheld", wert: 1 }
  - { aspekt: "Über-crunchiges Zahlensystem", wert: -1 }
  - { aspekt: "Thomas Nicolai", wert: -1 }
cover: ""
quellen:
  - "https://www.audible.de/series/Die-boesen-Jungs-Hoerbuecher/B0C4YSTXWW"
  - "https://www.audible.de/pd/Schurken-Halunken-Hoerbuch/B0C3CXJFHT"
  - "https://www.fantasticfiction.com/u/eric-ugland/bad-guys/"
  - "https://www.amazon.de/Schurken-Halunken-Fantasy-LitRPG-GameLit-Roman-b%C3%B6sen-ebook/dp/B095X52FLR"
analysiert: 2026-08-11
# --- 🔒 Spoiler-Grenze (siehe _System/Spoiler-Politik.md) ---
spoiler_erlebt_bis: 0    # letzter VOLLSTÄNDIG erlebter Band (0 = keiner)
spoiler_aktuell: 1       # laufender Band — zählt als NICHT erlebt
spoiler_gesamt: 11        # Bd. 1 laeuft (2026-08-11) -> noch NICHTS abgeschlossen
spoiler_stand: "2026-08-11"  # Stand bestätigt am
spoiler_sperrbegriffe: []    # NUR neutrale Begriffe jenseits der Grenze, NIE das Ereignis
spoiler_unerlebte_reihen: ["The Grim Guys"]  # Reihen im selben Universum, NIE erlebt -> immer nur Klappentext-Ebene
spoiler_aliase: []           # weitere Schreibweisen des Werktitels
tags: [werk, reihe, hoerbuch, liked, litrpg]
---

# Die bösen Jungs (The Bad Guys)

**Eigenständige Reihe**, untergeordnet der Overall-Ebene [[Vuldranni (iNcarn8)]]
(Overall → Reihe). **Ben**, ein kleiner Trickbetrüger aus New York, stirbt und wacht in
Vuldranni mit neuem Gesicht und neuem Namen auf: **Clyde Hatchett**, gelandet in der
Reichshauptstadt **Glaton**. Er will seine Diebeskarriere neu starten und das „Spiel" als
Robin-Hood-Typ spielen — dieselbe Welt wie [[Die guten Jungs]], nur von der **anderen Seite
der Moral**. 11 Bände erschienen (2019–2024, „Schurken & Halunken“ bis „2 Lügen, 2 Throne“).
⚠️ **Korrektur 2026-08-11: NICHT abgeschlossen — Bd. 12 ist für 2026 angekündigt.**

**Kontext:** [[LitRPG]] · **Autor:** [[Eric Ugland]] · **Sprecher:** [[Thomas Nicolai]] ·
**Parallelreihe:** [[Die guten Jungs]]

## 📚 Die Bände einzeln

Deutsch bei **LMBPN International** (Übers. **Simone Förster**, Bd. 1 zusätzlich
**Moritz Hirsch**), Hörbuch **Audible Studios** / [[Thomas Nicolai]].

⚠️ **Gleiche Quellenlage wie bei der Schwesterreihe:** Audible.de, BücherTreff, Goodreads,
Amazon und DNB sind in dieser Session Egress-geblockt — nur Websuche. Titel ohne Marker
sind aus einer echten Seitenüberschrift/URL belegt, **⚠️** markiert nur abgeleitete Angaben.

| # | 🇩🇪 Titel | 🇬🇧 Original (Jahr) | 🎧 dt. Hörbuch | Lesestand |
| --- | --- | --- | --- | --- |
| 1 | Schurken & Halunken | Scamps & Scoundrels (2019) | **15 h 23** · 2023 · ASIN B0C3CYC868 / B0C3CXJFHT | ▶️ **läuft gerade** |
| 2 | Der Dieb im ersten Stock | Second Story Man (2019) | ✓ erschienen · ASIN B0C4YHJQT2 · Dauer ⚠️ | offen |
| 3 | Die Freischaufler | Skull and Thrones (2020) | ⚠️ offen | offen |
| 4 | Krieg der Aufschneider | War of the Posers (2020) | ⚠️ offen | offen |
| 5 | Seeungeheuer und andere Kalamitäten ⚠️ | Seas the Day (2020) | ⚠️ offen | offen |
| 6 | Unterm Arsch der Welt, und dann links ⚠️ | High Gloom (2020) | ⚠️ offen | offen |
| 7 | Zurück auf Eins | Back to One (2021) | ✓ erschienen · ASIN B0DWMVVFPZ (2025) | offen |
| 8 | Spaß in der Nacht | Trick of the Night (2022) | ✓ erschienen · ASIN B0DXQ14LFX (2025) | offen |
| 9 | ⚠️ dt. Titel unbestätigt (evtl. „Darktown Funk" übernommen) | Darktown Funk (2022) | ⚠️ offen | offen |
| 10 | Auf einem Thron aus Lügen | On a Throne of Lies (2023) | ⚠️ offen · Print TB 496 S. (2024) | offen |
| 11 | **2 Lügen, 2 Throne** | **2 Lies, 2 Thrones (2024)** 🔀 Crossover-Vorbereitung | ⚠️ offen | offen |

**Bandlängen — wichtiger Unterschied zur Schwesterreihe:** Bd. 1 ist mit **15 h 23** fast
doppelt so lang wie die kurzen Bände der [[Die guten Jungs]] (7 h 42 – 9 h 39). Noch im
Rahmen (≤ ~20 h, [[No-Gos]]), aber die Reihe hört sich deutlich „schwerer" an — relevant
beim verschränkten Hören, weil sich das Tempo-Gefühl beim Wechsel ändert.

### Offene Punkte zu den Bänden

- Bd. 9: ein Suchtreffer-Fließtext nennt „Darktown Funk" als **deutschen** Titel (Amazon.de,
  4,6 · 733 Bew.) — der Wert konnte nicht am Original belegt werden. **Offen.**
- Bd. 3 „Die Freischaufler" ↔ EN „Skull and Thrones": Zuordnung positionsbasiert (die
  1:1-Bandzuordnung DE↔EN ist bei Bd. 7, 10 und 11 durch wörtliche Titelübereinstimmung
  belegt und damit für die Reihe insgesamt gut gestützt).
- ISBNs (außer Bd. 1 Print **978-1-64971-236-3**), Seitenzahlen, Wertungen je Band: offen.

## 🗓️ Nächste Releases & Planung

- **Buch DE:** 11 Bände erschienen. ⚠️ **Bd. 12 für 2026 angekündigt.**
- **Hörbuch DE:** ⚠️ **belegt bis Bd. 8** (2025). Für die verschränkte Hörreihenfolge ist das
  der **engere** Flaschenhals — die bösen Jungs werden vermutlich zuerst „ausgehen".
- **EN:** ⚠️ **Bd. 12 für 2026 angekündigt** — Reihe läuft weiter (letzter Band: Bd. 11, 2024).

## Lesestand (meine Formate)

| Bd. | Status | Notiz |
| --- | --- | --- |
| 1 | ▶️ läuft (dt., [[Thomas Nicolai]]) | „bj bin ich grade bei 1" (2026-08-11) |
| 2–11 | offen | laut Hörreihenfolge als Nächstes Bd. 2, dann zurück zu [[Die guten Jungs]] 6 |

## ⭐ Bewertung je Aspekt

> ⛔ **BEWERTUNG AUSGESETZT — Nutzer-Entscheidung 2026-08-11, wörtlich:**
> „**Böse Jungs kann ich erst nach Ende von Band 1 was zu sagen.**"
>
> **Nicht nach einer Meinung zu dieser Reihe fragen, solange Bd. 1 läuft.** Was unten steht,
> ist ausdrücklich **nicht** vom Nutzer bestätigt: die *geerbten* Zeilen stammen aus dem
> Verbund [[Vuldranni (iNcarn8)]], die reihen-eigenen sind Reihenanlage bzw. eine frühe
> Ersteindruck-Aussage. **Einziger belastbarer reihen-eigener Befund: der Sprecher**
> ([[Thomas Nicolai]] −1) — den hat der Nutzer direkt und begründet verglichen.
>
> ➡️ **Wiedervorlage:** Sobald `spoiler_erlebt_bis: 1` erreicht ist (Bd. 1 durch), das volle
> Interview nach `book-taste-interview` fahren — Abdeckungs-Matrix von Anfang an mitführen,
> Positiv/Negativ getrennt. Vorher **kein** Verdikt verfestigen.

| Aspekt | Wertung | Warum | Beleg |
| --- | --- | --- | --- |
| *geerbt:* [[Absurder Humor]] | +2 | Ugland-Handschrift, gilt für beide Reihen | geerbt aus [[Vuldranni (iNcarn8)]] — „Humor/Ton zündet" |
| *geerbt:* [[Tonwechsel Humor-Ernst]] | +2 | s. o. | geerbt aus [[Vuldranni (iNcarn8)]] |
| [[Pragmatischer Held mit klarem Kompass]] | +2 ⚠️ **ungesichert** | Clyde ist handlungsfähig, kein Jammerlappen | „Held ist stark" — Ersteindruck aus Runde 1, **galt beiden Reihen pauschal**; für BJ nicht eigens bestätigt (s. Sperre oben) |
| *geerbt:* [[Stimmige Weltatmosphäre]] | +2 | dieselbe Welt Vuldranni | geerbt aus [[Vuldranni (iNcarn8)]] |
| [[Moralisch ambivalenter Antiheld]] | +1 | Dieb-POV statt Helden-POV — Reiz der Gegenseite | Reihenanlage; ⚠️ **vorläufig**, Bd. 1 läuft noch |
| *geerbt:* [[Über-crunchiges Zahlensystem]] | −1 | Status-/Skill-Vorlesen | „nur ein mini negativer punkt" |
| 🎙️ [[Thomas Nicolai]] | **−1** | Stimmfarbe passt nicht zur Figur, Tempo/Betonung schwächer | „Stimmfarbe passt nicht" + „Tempo & Betonung"; ausdrücklich **„nur im Vergleich schwächer"** |

**Verdikt-Herleitung:** **`liked` — sehr vorläufig.** Der Nutzer ist **mitten in Bd. 1 von
11**; belastbar sind bisher nur (a) die aus dem Verbund geerbten Plus-Aspekte und (b) der
einzige reihen-eigene Befund: **der Sprecher fällt gegen [[Robert Frank]] ab**. Das ist ein
Hörbuch-Malus, **kein Werk-Malus** — inhaltlich ist die Reihe unbelastet
(siehe [[Konventionen]]: Sprecher ist eine **zusätzliche** Dimension, kein Matching-Faktor).
Vollbewertung folgt, sobald mindestens Bd. 1 durch ist.

## Abweichungen einzelner Bände (vom Nutzer selbst genannt)

*(keine)*

## DNA (das Eigene)

- **Figuren:** Clyde Hatchett — [[Moralisch ambivalenter Antiheld]] + [[Pragmatischer Held
  mit klarem Kompass]]; Dieb/Schurken-Perspektive statt Aufbauer
- **Setting:** Reichshauptstadt **Glaton** (Stadt/Unterwelt) statt Wildnis/Herzogtum
- **Länge:** deutlich längere Bände als die Schwesterreihe (Bd. 1: 15 h 23)
- *geerbt aus [[Vuldranni (iNcarn8)]]:* Welt, Ton, Systemlogik (Pflege nur an der Quelle!)

## 🔀 Verhältnis zu [[Die guten Jungs]]

**Eigenständig hörbar** — aber die Community-Empfehlung ist eindeutig: erst die guten Jungs,
weil sie „ein gutes Hintergrundwissen über die Welt Vuldranni und ihre politischen
Entwicklungen" liefern. Weltereignisse werden **aus zwei Perspektiven** erzählt. Der
Zusammenlauf: **Bd. 11 „2 Lügen, 2 Throne" muss VOR [[Die guten Jungs]] Bd. 15 „Kampf um den
Keramikthron"** gehört werden. Volle Reihenfolge: [[Vuldranni (iNcarn8)]].

## 🧮 Reihen-Gesamtbild (letzter Schritt)

⚠️ **Kein belegter Serien-Durchschnitt** (Goodreads/Audible geblockt). Community-Tenor aus
Rezensions-Snippets: die Reihe gilt manchen als „potenziell besser als die guten Jungs" —
voller Loot, Magie und Intrigen; der Sprecher [[Thomas Nicolai]] wird in dt. Rezensionen
durchaus gelobt („kann mit Durchstimmvarianz sehr überzeugen"), was den Nutzer-Malus als
**Geschmacksurteil im direkten Vergleich** einordnet, nicht als Qualitätsmangel.

## Offene Fragen

- ⚠️ Serien-Rating + Stimmenzahl, Wertungen je Band.
- ⚠️ dt. Titel Bd. 5, 6, 9 bestätigen; Hördauern ab Bd. 2; Hörbuch-Stand ab Bd. 9.
- **An den Nutzer (sobald Bd. 1 durch):** Trägt Clyde als Antiheld genauso wie Montana?
  Fühlt sich das Stadt-/Diebes-Setting besser oder schlechter an als der Aufbau in Coggeshall?
  Und: stört dich die deutlich längere Bandlänge (15 h vs. 9 h)?

Quelle Interview: [[2026-08-11 Interview Die guten Jungs & Die bösen Jungs]]
